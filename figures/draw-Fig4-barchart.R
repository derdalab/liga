# routine to run the process data files to produce Figure 4 bar charts
library(ggplot2)
library(grid)

# set some parameters
data.dir <- "LiGA data for figures"
filename.pattern <- "Figure4.*"

# set the output file
# height and width are in inches, 7 inches is about a two-column figure width
cairo_pdf(file = "Figure4-barchart.pdf", width = 5, height = 7)
...
for(subpanel in 1:length(diff.tests)){
    ...
    bar_plot <- ggplot(data = plot_data,
            ...
            # uncomment the next two lines to add error bars
    ...
    # include the label text under the last panel only
    if(subpanel >= length(diff.tests)){
