...
cairo_pdf(file = "FigureS13B-heatmap.pdf", width = 7, height = 4.325)
     # function cairo_pdf() can be replaced with pdf() but lacks unicode support
...
for(filenumber in 1:length(input_filename_list)){
    ...
    # read the file
    file.data <- read.table(file = filename, header = TRUE, sep = "\t",
                            quote = "\"", check.names = FALSE)
 
    # append the data need for plotting to the long table
    long.data <- rbind(
        long.data,
        data.frame(Glycan.Rank = 1:NROW(file.data),
                   Glycan.Label = factor(file.data$Barcode,
                               levels = unique(file.data$Barcode), ordered = TRUE),
                   Case.Rank = filenumber,
                   Case.Label = factor(file.data$Type,
                               levels = unique(file.data$Type), ordered = TRUE),
                   logFC = file.data$logFC,
                   PValue = file.data$PValue)
        )
    # in this case the ranks are correctly embedded in the factors so extract them
    long.data$Glycan.Rank = as.integer(long.data$Glycan.Label)
    long.data$Case.Rank = as.integer(long.data$Case.Label)
}
...
heatmap <- ggplot(long.data,
  ...
  geom_text(label = "\u25e6", hjust = 0.5, vjust = 0.55, color = "#808080",
  ...
  theme(axis.text.x  = element_text(face = "bold", size = 5.8,
                                    hjust = 1, vjust = 0.5, angle = 90),
        axis.text.y  = element_text(face = "bold", size = 5.8, hjust = 0),
        legend.position = "top",
