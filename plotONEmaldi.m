%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
addpath (fullfile('/Volumes/Data/!! OUTLINES/Jasmine - LiGA/MALDI/'));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%


%filename = '340.txt';
% names = 'TriAN3';
%full_temp = 'Gala1-3[Fuca1-2]Galb-Sp';

%isCapped = 0;  % set to 1 if azidoethanol capping was used. 
%MW_DBCO = 287; %Use from 315 for "old DBCO" and 287 for new DBCO


% change the directory name for your computer to point to MALDI-names.xlsx%
D = '/Users/mirat/Dropbox/Database';
%D = '/Users/derdalab/Documents/Dropbox/Database';
%D = '/Users/ratmir/Dropbox/Database';
%D = '/Users/ratmirderda/Dropbox/Database';

F = 'MALDI-names.xlsx';

% keep it blank if you don't want to save anything
SaveDir = '/Users/mirat/Desktop/stufff';

% this file will be saved in your present working directory unless you
% define it otherwise.
FID = fopen(fullfile(SaveDir,'densities.csv'), 'w');

[~,~,temp] = xlsread(fullfile(D,F));

whereGlycanName = find(strcmp('Glycan Name', temp(1,:) ));
whereCFGName = find(strcmp('CFG Name', temp(1,:) ));


% you can play with the lines here
for LINE = 419:419 %1:size(temp,1)
    
    %waitforbuttonpress   %uncomment this to be able to pause at each plot
    manual = 0; 
    
if isnumeric(temp{LINE,2})
    filename = [num2str(temp{LINE,2}) '.txt'];
else
    filename = [temp{LINE,2} '.txt'];
end

names = temp{LINE,whereGlycanName};
full_temp = temp{LINE,whereCFGName};


if isempty(temp{LINE,8}) | isnan(temp{LINE,8}) 
    MW_DBCO = 287; % assume that if its empty, its a new DBCO
else
    MW_DBCO = temp{LINE,8};
end

if isempty(temp{LINE,10}) | isnan(temp{LINE,10}) 
    isCapped = 0;
else
    isCapped = temp{LINE,10};
end

if isempty(temp{LINE,9}) | isnan(temp{LINE,9}) 
    RANGE = 200;
else
    RANGE = temp{LINE,9};
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%        
    ...
    if ~isempty(SaveDir)
        print(h, '-depsc', fullfile(SaveDir,[filename(1:end-4) '.eps']) );
        print(h, '-djpeg', fullfile(SaveDir,[filename(1:end-4) '.jpeg']) );
    end
    
    if ~isempty(Ratio)
        fprintf(FID, '%s', [filename ',' num2str(sum(Ratio)) char(10)  ]);
        disp('im here');
    else
        fprintf(FID, '%s', [filename ',' 'problem' char(10)  ]);
        disp('im here 2');
    end
end %of uber super duper for loop
